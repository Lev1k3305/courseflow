...
/**
 * Deterministic hash function for avatar seeds to prevent redundant image reloads
 */
const getAvatarSeed = (courseId: string, index: number) => {
  let hash = 0;
  for (let i = 0; i < courseId.length; i++) {
    hash = (hash << 5) - hash + courseId.charCodeAt(i);
    hash |= 0;
  }
  return Math.abs(hash + index) % 70;
};
          ...
          <div className="flex -space-x-2">
             {avatarIndices.map((i) => (
               <div key={i} className="w-7 h-7 rounded-full border-2 border-white dark:border-zinc-900 bg-zinc-200 dark:bg-zinc-800 overflow-hidden">
                 <img
                    src={`https://i.pravatar.cc/100?img=${getAvatarSeed(course.id, i)}`}
                    alt="User"
                    loading="lazy"
                    width={28}
                    height={28}
